...
var (
	...
	appPath         = filepath.Join(os.Getenv("LOCALAPPDATA"), "Programs", "Luna AI")
	appLogPath      = filepath.Join(os.Getenv("LOCALAPPDATA"), "Luna AI", "app.log")
	startupShortcut = filepath.Join(os.Getenv("APPDATA"), "Microsoft", "Windows", "Start Menu", "Programs", "Startup", "Luna AI.lnk")
	lunaPath        string
	DesktopAppName  = "Luna AI.exe"
)
...
func init() {
	...
	lunaPath = filepath.Join(appPath, "luna-ai.exe")

	// Handle developer mode (go run ./cmd/app)
	if _, err := os.Stat(lunaPath); err != nil {
		pwd, err := os.Getwd()
		if err != nil {
			slog.Warn("missing luna-ai.exe and failed to get pwd", "error", err)
			return
		}
		distAppPath := filepath.Join(pwd, "dist", "windows-"+runtime.GOARCH)
		distLunaPath := filepath.Join(distAppPath, "luna-ai.exe")
		if _, err := os.Stat(distLunaPath); err == nil {
			slog.Info("detected developer mode")
			appPath = distAppPath
			lunaPath = distLunaPath
		}
	}
}
...
func createLoginShortcut() error {
	...
	shortcutOrigin := filepath.Join(appPath, "lib", "Luna AI.lnk")
...
func logStartup() {
	slog.Info("starting Luna AI", "app", appPath, "version", version.Version, "OS", updater.UserAgentOS)
}
...
func runInBackground() {
	...
	if cmd != nil {
		err = cmd.Run()
		if err != nil {
			slog.Error("failed to run Luna AI", "exe", exe, "error", err)
			os.Exit(1)
		}
	} else {
		slog.Error("failed to start Luna AI", "exe", exe)
		os.Exit(1)
	}
}
