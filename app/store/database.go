...
func (db *database) init() error {
	...
	CREATE TABLE IF NOT EXISTS chats (
		...
		browser_state TEXT,
		private_mode BOOLEAN NOT NULL DEFAULT 0
	);
...
func (db *database) migrate() error {
	...
	for version < currentSchemaVersion {
		...
		case 11:
			...
			version = 12
		case 12:
			// add nickname, avatar_path, and private_mode
			if err := db.migrateV12ToV13(); err != nil {
				return fmt.Errorf("migrate v12 to v13: %w", err)
			}
			version = 13
		case 13:
			// add custom_instructions, appearance, and accent_color
...
// migrateV12ToV13 adds nickname, avatar_path to settings and private_mode to chats
func (db *database) migrateV12ToV13() error {
	_, err := db.conn.Exec(`ALTER TABLE settings ADD COLUMN nickname TEXT NOT NULL DEFAULT ''`)
	if err != nil && !duplicateColumnError(err) {
		return fmt.Errorf("add nickname column: %w", err)
	}

	_, err = db.conn.Exec(`ALTER TABLE settings ADD COLUMN avatar_path TEXT NOT NULL DEFAULT ''`)
	if err != nil && !duplicateColumnError(err) {
		return fmt.Errorf("add avatar_path column: %w", err)
	}

	_, err = db.conn.Exec(`ALTER TABLE chats ADD COLUMN private_mode BOOLEAN NOT NULL DEFAULT 0`)
	if err != nil && !duplicateColumnError(err) {
		return fmt.Errorf("add private_mode column: %w", err)
	}
...
// migrateV14ToV15 adds primary_color to settings
func (db *database) migrateV14ToV15() error {
	_, err := db.conn.Exec(`ALTER TABLE settings ADD COLUMN primary_color TEXT NOT NULL DEFAULT ''`)
	if err != nil && !duplicateColumnError(err) {
		return fmt.Errorf("add primary_color column: %w", err)
	}
...
func (db *database) getChatWithOptions(id string, loadAttachmentData bool) (*Chat, error) {
	query := `
		SELECT id, title, created_at, browser_state, private_mode
		FROM chats
		WHERE id = ?
	`
...
func (db *database) saveChat(chat Chat) error {
	...
	query := `
		INSERT INTO chats (id, title, created_at, browser_state, private_mode)
		VALUES (?, ?, ?, ?, ?)
...
func (db *database) deletePrivateChats() error {
	_, err := db.conn.Exec("DELETE FROM chats WHERE private_mode = 1")
	if err != nil {
		return fmt.Errorf("delete private chats: %w", err)
	}

	// Also clean up related messages/attachments via foreign keys/orphaned logic
	// But immediate WAL checkpoint ensures it's committed
	_, _ = db.conn.Exec("PRAGMA wal_checkpoint(TRUNCATE);")

	return nil
}

func (db *database) branchChat(sourceID string, destID string, messageIndex int) error {
	tx, err := db.conn.Begin()
	if err != nil {
		return fmt.Errorf("begin transaction: %w", err)
	}
	...
	// 1. Copy Chat Metadata
	// We get the source chat title and privacy setting, but generate a new ID and timestamp
	var title string
	var privateMode bool
	err = tx.QueryRow("SELECT title, private_mode FROM chats WHERE id = ?", sourceID).Scan(&title, &privateMode)
	if err != nil {
		return fmt.Errorf("get source chat metadata: %w", err)
	}

	// Insert new chat
	_, err = tx.Exec(`
		INSERT INTO chats (id, title, created_at, private_mode)
		VALUES (?, ?, ?, ?)
	`, destID, title, time.Now(), privateMode)
	if err != nil {
		return fmt.Errorf("create branched chat: %w", err)
	}

	// 2. Fetch Source Messages up to index
	// We need to fetch them in order to re-insert them
	rows, err := tx.Query(`
	...
	`, sourceID)
	if err != nil {
		return fmt.Errorf("query source messages: %w", err)
	}
	defer rows.Close()
	...
	currentIndex := 0
	for rows.Next() {
		// Stop if we've passed the target index
		...
		// Scan into temporary vars for nullable fields
		var id int64 // unused
		err := rows.Scan(
			&id,
			&msg.Role,
		...
		if thinkingTimeStart.Valid {
			t := thinkingTimeStart.Time
			msg.ThinkingTimeStart = &t
		}
		if thinkingTimeEnd.Valid {
			t := thinkingTimeEnd.Time
			msg.ThinkingTimeEnd = &t
		}
		if toolResult.Valid {
			raw := json.RawMessage(toolResult.String)
			msg.ToolResult = &raw
		}

		sourceMessages = append(sourceMessages, msg)
		currentIndex++
	}

	// 3. Insert Messages into New Chat
	for _, msg := range sourceMessages {
		_, err := db.insertMessage(tx, destID, msg)
		if err != nil {
			return fmt.Errorf("insert branched message: %w", err)
		}
		// Note: We are strictly copying message content. Deep copying attachments/tools
		// would require recursively fetching and inserting them.
		// For MVP "Orbit", we rely on the fact that standard text history is the priority.
		// If attachments need to move, we'd need to copy those rows too.
		// TODO: Add attachment/tool_call copying if complex history branching is needed.
	}
...
func (db *database) getSettings() (Settings, error) {
	...
	err := db.conn.QueryRow(`
		SELECT expose, survey, browser, models, agent, tools, working_dir, context_length, airplane_mode, turbo_enabled, websearch_enabled, selected_model, sidebar_open, think_enabled, think_level, nickname, avatar_path, custom_instructions, appearance, accent_color, primary_color
		FROM settings
	`).Scan(&s.Expose, &s.Survey, &s.Browser, &s.Models, &s.Agent, &s.Tools, &s.WorkingDir, &s.ContextLength, &s.AirplaneMode, &s.TurboEnabled, &s.WebSearchEnabled, &s.SelectedModel, &s.SidebarOpen, &s.ThinkEnabled, &s.ThinkLevel, &s.Nickname, &s.AvatarPath, &s.CustomInstructions, &s.Appearance, &s.AccentColor, &s.PrimaryColor)
	if err != nil {
...
func (db *database) setSettings(s Settings) error {
	_, err := db.conn.Exec(`
		UPDATE settings 
		SET expose = ?, survey = ?, browser = ?, models = ?, agent = ?, tools = ?, working_dir = ?, context_length = ?, airplane_mode = ?, turbo_enabled = ?, websearch_enabled = ?, selected_model = ?, sidebar_open = ?, think_enabled = ?, think_level = ?, nickname = ?, avatar_path = ?, custom_instructions = ?, appearance = ?, accent_color = ?, primary_color = ?
	`, s.Expose, s.Survey, s.Browser, s.Models, s.Agent, s.Tools, s.WorkingDir, s.ContextLength, s.AirplaneMode, s.TurboEnabled, s.WebSearchEnabled, s.SelectedModel, s.SidebarOpen, s.ThinkEnabled, s.ThinkLevel, s.Nickname, s.AvatarPath, s.CustomInstructions, s.Appearance, s.AccentColor, s.PrimaryColor)
	if err != nil {
...
// migrateV13ToV14 adds custom_instructions, appearance, and accent_color to settings
func (db *database) migrateV13ToV14() error {
	_, err := db.conn.Exec(`ALTER TABLE settings ADD COLUMN custom_instructions TEXT NOT NULL DEFAULT ''`)
	if err != nil && !duplicateColumnError(err) {
		return fmt.Errorf("add custom_instructions column: %w", err)
	}

	_, err = db.conn.Exec(`ALTER TABLE settings ADD COLUMN appearance TEXT NOT NULL DEFAULT ''`)
	if err != nil && !duplicateColumnError(err) {
		return fmt.Errorf("add appearance column: %w", err)
	}

	_, err = db.conn.Exec(`ALTER TABLE settings ADD COLUMN accent_color TEXT NOT NULL DEFAULT ''`)
	if err != nil && !duplicateColumnError(err) {
		return fmt.Errorf("add accent_color column: %w", err)
	}
