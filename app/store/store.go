...
type Settings struct {
	// Expose tells the ollama server to listen on all interfaces (using OLLAMA_HOST)
	Expose bool `json:"expose"`

	// Browser when true, tells the desktop app to open the browser on startup
	Browser bool `json:"browser"`

	// Survey indicates if the user has completed the survey
	Survey bool `json:"survey"`

	// Models specifies the path to the models directory (using OLLAMA_MODELS)
	Models string `json:"models"`

	// Agent specifies the path to the agent binary
	Agent bool `json:"agent"`

	// Tools specifies if tools are enabled
	Tools bool `json:"tools"`

	// WorkingDir specifies the path to the working directory
	...
	WebSearchEnabled bool `json:"websearch_enabled"`
	...
	// Nickname for the user profile
	Nickname string `json:"nickname"`

	// AvatarPath for the user profile
	AvatarPath string `json:"avatar_path"`

	// CustomInstructions for the AI
	CustomInstructions string `json:"custom_instructions"`

	// Appearance settings (placeholder for future expansion)
	Appearance string `json:"appearance"`

	// AccentColor settings (placeholder for future expansion)
	AccentColor string `json:"accent_color"`

	// PrimaryColor for the main application theme
...
func (s *Store) DeletePrivateChats() error {
	if err := s.ensureDB(); err != nil {
		return err
	}

	// Delete from database
	if err := s.db.deletePrivateChats(); err != nil {
...
func (s *Store) BranchChat(sourceID string, destID string, messageIndex int) error {
	if err := s.ensureDB(); err != nil {
		return err
	}

	return s.db.branchChat(sourceID, destID, messageIndex)
}
