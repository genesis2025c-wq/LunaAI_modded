...
export async function fetchUser(): Promise<User | null> {
  // Return a local stub user instead of calling the backend for Ollama cloud info
  ...
  if (response.ok) {
    ...
    return {
      id: "local-user",
      name: settings.nickname || "Luna User",
      nickname: settings.nickname || "Luna User",
      email: "local@luna-ai",
      plan: "free",
  ...
  const response = await fetch(`${API_BASE}/api/v1/chat/${chatId}`, {
    ...
    body: JSON.stringify(
      new ChatRequest({
        ...
        private_mode: privateMode ?? false,
...
export async function branchChat(
  ...
  const response = await fetch(`${API_BASE}/api/v1/chat/${sourceId}/branch`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  if (!response.ok) {
    const error = await response.text();
    throw new Error(error || "Failed to branch chat");
  }
