...
export function useSelectedModel(currentChatId?: string, searchQuery?: string) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      setSettings({ selected_model: selectedModel.model });
    }
    ...
    if (
      ...
      setSettings({ selected_model: selectedModel.model, turbo_enabled: false });
    }
      ...
      for (const message of messages) {
        if (message.model) {
          ...
          if (chatModelName !== settings.selectedModel) {
            setSettings({ selected_model: chatModelName });
          }
  ...
  useEffect(() => {
    ...
    if (defaultModel) {
      setSettings({ selected_model: defaultModel.model });
    }
  }, [
    isLoading,
