...
>(function ModelPicker(
  ...
  const handleModelSelect = (model: Model) => {
    setSettings({ selected_model: model.model });
    setIsOpen(false);
    onModelSelect?.();
  };
          ...
          return (
            <div key={`${model.model}-${model.digest || "no-digest"}-${index}`}>
              <button
                ...
                className={`flex w-full items-center gap-2 px-3 py-2 hover:bg-neutral-100 dark:hover:bg-neutral-700/60 focus:outline-none cursor-pointer ${highlightedIndex === index ||
