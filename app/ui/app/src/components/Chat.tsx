...
import privateIcon from "@/assets/icon_private.png";
...
export default function Chat({ chatId }: { chatId: string }) {
  ...
  useEffect(() => {
    // Check URL parameters for new chats
    const searchParams = new URLSearchParams(window.location.search);
    setIsPrivate(searchParams.get("private") === "true");
  }, [chatId, window.location.search]);

  // Determine effective private mode (URL for new, Chat Data for existing)
  const isChatPrivate = chatQuery?.data?.chat?.private_mode || false;
  const effectivePrivateMode = chatId === "new" ? isPrivate : isChatPrivate;
      ...
      {chatId === "new" ? (
        <div className="flex flex-col h-screen justify-center relative">
          <div className="px-6">
            <ChatForm
              ...
              initialPrivateMode={effectivePrivateMode}
            />
          </div>
        </div>
      ) : (
        <main className="flex h-screen w-full flex-col relative allow-context-menu select-none">
          {effectivePrivateMode && (
            <div className="bg-neutral-800 text-neutral-300 px-4 py-2 flex items-center justify-center gap-2 text-sm font-medium border-b border-neutral-700">
              <img src={privateIcon} className="h-4 w-4 dark:invert" alt="Private" />
