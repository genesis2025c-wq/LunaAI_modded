  ...
  return (
    ...
    >
      ...
      >
        {/* Browser icon */}
        <svg
          className={`w-10 absolute -left-0.5 top-1 transition-opacity ${isCollapsed ? "opacity-100" : "opacity-0"
        ...
        {/* Arrow */}
        <svg
          className={`h-4 w-4 absolute top-1.5 transition-all ${isCollapsed
      ...
      {rawToolResult && (
        <div className="flex flex-col w-full text-neutral-600 dark:text-neutral-400 relative select-text hover:text-neutral-800 dark:hover:text-neutral-200 transition-colors">
          ...
          >
            ...
            {/* Arrow */}
            <svg
              className={`h-4 w-4 absolute top-1.5 transition-all opacity-0 group-hover/browser:opacity-100 ${isCollapsed ? "-rotate-90" : "rotate-0"
                } will-change-[opacity,transform]`}
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
  ...
  return (
    ...
    >
      ...
      >
        {/* Tool icon */}
        <svg
          className={`w-3 absolute left-0 top-1/2 -translate-y-1/2 transition-opacity ${isCollapsed ? "opacity-100" : "opacity-0"
        ...
        {/* Arrow */}
        <svg
          className={`h-4 w-4 absolute transition-all ${isCollapsed
...
function UserMessage({
  ...
  const { user } = useUser();
  const nickname = settingsData?.nickname || user?.nickname || "User";
  const avatarPath = settingsData?.avatar_path;
    ...
    >
      <div className="flex items-center gap-2 mb-1 self-end mr-1">
        ...
        {avatarPath ? (
          ...
          <div className="w-5 h-5 rounded-full bg-neutral-200 dark:bg-neutral-800 flex items-center justify-center">
...
function OtherRoleMessage({
  ...
  const handleOrbit = async () => {
    ...
    setIsBranching(true);
    try {
      const { id: newChatId } = await branchChat(chatId, messageIndex);
      navigate({ to: "/c/$chatId", params: { chatId: newChatId } });
    } catch (err) {
      console.error("Failed to orbit:", err);
    } finally {
          ...
          <div className="-ml-1 flex items-center gap-1">
            ...
            {messageIndex !== undefined && (
              <button
                ...
                className={`p-1 rounded hover:bg-neutral-100 dark:hover:bg-neutral-800 text-neutral-500 dark:text-neutral-400 transition-colors ${isBranching ? "opacity-50 cursor-wait" : ""}`}
                title="Orbit (Branch Chat)"
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-split-icon lucide-split w-4 h-4"><path d="M16 3h5v5" /><path d="M8 3H3v5" /><path d="M12 22v-8.3a4 4 0 0 0-1.172-2.872L3 3" /><path d="m15 9 6-6" /></svg>
              </button>
            )}
          </div>
        )}
