...
export function ChatSidebar({ currentChatId }: ChatSidebarProps) {
  ...
  const isPrivate = search.private === true || search.private === "true";
  ...
  return (
    <nav className="flex flex-1 flex-col min-h-0 select-none">
      <header className="flex flex-col gap-0.5 px-4 pb-4 pt-4">
        <div className="flex items-center gap-2 px-2 mb-2">
          <div className="w-6 h-6 rounded-md bg-black dark:bg-white flex items-center justify-center">
            <svg viewBox="0 0 24 24" className="w-4 h-4 text-white dark:text-black fill-current">
              <path d="M12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22C17.5228 22 22 17.5228 22 12C22 11.4373 21.9535 10.8855 21.8638 10.349C20.6548 11.3789 19.1001 12 17.4 12C13.3681 12 10.1 8.73188 10.1 4.7C10.1 3.6339 10.3292 2.62128 10.7428 1.70886C10.3541 1.63604 9.95427 1.6 9.55 1.6L12 2Z" />
            </svg>
          </div>
          <span className="font-rounded font-bold text-lg dark:text-white">Luna AI</span>
        </div>
        ...
        <Link
          ...
          search={{}}
          mask={{ to: "/" }}
          className={`flex w-full items-center gap-3 rounded-lg px-2 py-2 text-left text-sm text-neutral-700 hover:bg-neutral-100 dark:hover:bg-neutral-800 dark:text-neutral-100 ${currentChatId === "new" && !isPrivate ? "bg-neutral-100 dark:bg-neutral-800" : ""
            }`}
          title="New Chat"
        >
          <svg
        ...
        <Link
          to="/c/$chatId"
          params={{ chatId: "new" }}
          search={{ private: true } as any}
          mask={{ to: "/" }}
          className={`flex w-full items-center gap-3 rounded-lg px-2 py-2 text-left text-sm text-neutral-700 hover:bg-neutral-100 dark:hover:bg-neutral-800 dark:text-neutral-100 ${currentChatId === "new" && isPrivate ? "bg-neutral-100 dark:bg-neutral-800" : ""
            }`}
          title="Private Chat"
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-hat-glasses-icon lucide-hat-glasses h-5 w-5"><path d="M14 18a2 2 0 0 0-4 0" /><path d="m19 11-2.11-6.657a2 2 0 0 0-2.752-1.148l-1.276.61A2 2 0 0 1 12 4H8.5a2 2 0 0 0-1.925 1.456L5 11" /><path d="M2 11h20" /><circle cx="17" cy="18" r="3" /><circle cx="7" cy="18" r="3" /></svg>
      ...
      <footer className="mt-auto px-4 py-4 border-t border-neutral-200 dark:border-neutral-800">
        <Link
          to="/settings"
          className="flex items-center gap-3 w-full p-2 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors"
          draggable={false}
        >
          {user?.avatar_path ? (
            <img
              src={`/api/v1/avatar?t=${Date.now()}`}
              alt="Profile"
            ...
            <UserCircleIcon className="h-8 w-8 text-neutral-400 dark:text-neutral-500" />
          )}
          <div className="flex flex-col min-w-0">
            <span className="text-sm font-medium text-neutral-900 dark:text-neutral-100 truncate">
              {user?.nickname || "Luna User"}
            </span>
