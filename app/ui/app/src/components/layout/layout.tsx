...
export function SidebarLayout({
  ...
  return (
    <div className={`flex transition-[width] duration-300 dark:bg-app-bg transition-colors duration-300`}>
      <div
      ...
      >
        <button
          onClick={() => setSettings({ sidebar_open: !settings.sidebarOpen })}
          onMouseDown={(e) => {
            e.stopPropagation();
          }}
        ...
        <Link
          ...
          params={{ chatId: "new" }}
          title="New chat"
          className={`flex ml-1 items-center justify-center rounded-full transition-opacity duration-375 h-9 w-9 hover:bg-neutral-100 dark:hover:bg-neutral-700 ${settings.sidebarOpen
      ...
      >
        <div
          className={`h-13 flex-none w-full z-10 flex items-center bg-white dark:bg-app-bg transition-colors duration-300 ${isWindows ? "xl:hidden" : "xl:fixed xl:bg-transparent xl:dark:bg-transparent"}`}
          onDoubleClick={() => window.doubleClick && window.doubleClick()}
          onMouseDown={() => window.drag && window.drag()}
        ></div>
        {children}
