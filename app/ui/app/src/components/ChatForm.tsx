...
function ChatForm({
  ...
  const [privateMode, setPrivateMode] = useState<boolean>(initialPrivateMode);
  ...
  const setThinkingLevel = (newLevel: ThinkingLevel) => {
    setSettings({ think_level: newLevel });
  };
  ...
  useEffect(() => {
    if (supportsThinkToggling && thinkEnabled && webSearchEnabled) {
      setSettings({ websearch_enabled: false });
    }
  }, [
    selectedModel?.model,
      ...
      >
        {privateMode && (
          <div className="absolute -top-3 left-1/2 -translate-x-1/2 z-10">
            <div className="bg-neutral-900 text-neutral-400 text-[10px] uppercase font-bold px-2 py-0.5 rounded-full border border-neutral-700 shadow-sm flex items-center gap-1">
              ...
              <div className={`flex gap-2`}>
                    ...
                    <ThinkButton
                      ...
                      onToggle={() => {
                        ...
                        if (supportsThinkToggling) {
                          ...
                          setSettings({
                            think_enabled: enable,
                            ...(enable ? { websearch_enabled: false } : {}),
                          });
                          return;
                        }
                        setSettings({ think_enabled: !thinkEnabled });
                      }}
                    />
                  </>
                )}
                <WebSearchButton
                  ...
                  onToggle={() => {
                    ...
                    setSettings({ websearch_enabled: enable });
                  }}
                />

                {/* Local Model (.gguf) Picker Button */}
                <button
                  type="button"
                  onClick={async () => {
                    if (window.webview?.selectFile) {
                      try {
                        const result = await window.webview.selectFile();
                        if (result && result.path.endsWith(".gguf")) {
                          // In a real app we might want to set this as the selected model
                          // For now, we'll just alert or could use it for the next message
                          setLocalModelName(result.path);
                          // We also need a way to set this as the active model in ModelPicker
                          // This is a bit complex since ModelPicker uses hooks.
                          // For now, let's just trigger a notification or similar.
                        }
                      } catch (error) {
                        console.error("Error selecting local model:", error);
                      }
                    }
                  }}
                  className={`select-none flex items-center justify-center rounded-full h-9 w-9 bg-white dark:bg-neutral-700 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer transition-all border border-transparent ${localModelName
                    ? "text-green-500 dark:text-green-400"
                    : "text-neutral-500 dark:text-neutral-400"
                    }`}
                  title="Load local .gguf model"
                >
                  <svg className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M20.25 6.375c0 2.223-1.812 4.031-4.031 4.031S12.188 8.598 12.188 6.375c0-2.223 1.812-4.031 4.031-4.031s4.031 1.808 4.031 4.031zm-15 0c0 2.223-1.812 4.031-4.031 4.031S1.188 8.598 1.188 6.375c0-2.223 1.812-4.031 4.031-4.031s4.031 1.808 4.031 4.031zm9 8.25c0 2.223-1.812 4.031-4.031 4.031s-4.031-1.808-4.031-4.031 1.812-4.031 4.031-4.031 4.031 1.808 4.031 4.031zm-9 0c0 2.223-1.812 4.031-4.031 4.031S1.188 16.848 1.188 14.625c0-2.223 1.812-4.031 4.031-4.031s4.031 1.808 4.031 4.031zm9 8.25c0 2.223-1.812 4.031-4.031 4.031s-4.031-1.808-4.031-4.031 1.812-4.031 4.031-4.031 4.031 1.808 4.031 4.031zm-9 0c0 2.223-1.812 4.031-4.031 4.031S1.188 25.098 1.188 22.875c0-2.223 1.812-4.031 4.031-4.031s4.031 1.808 4.031 4.031z" />
                  </svg>
                </button>
              </div>
            </div>
