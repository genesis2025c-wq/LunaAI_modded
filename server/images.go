...
func GetModel(name string) (*Model, error) {
	...
	// FORCE FIX: Inject Llama 3 template if missing or incorrect
	if strings.Contains(strings.ToLower(model.Name), "llama-3") && strings.Contains(strings.ToLower(model.Name), "instruct") {
		if model.Template == nil || model.Template.String() == "{{ .Prompt }}" || !strings.Contains(model.Template.String(), "<|start_header_id|>") {
			slog.Info("Injecting Llama 3 chat template", "model", model.Name)
			tmpl, err := template.Parse("{{- if .System }}<|begin_of_text|><|start_header_id|>system<|end_header_id|>\n\n{{ .System }}<|eot_id|>{{ end }}{{ range .Messages }}<|start_header_id|>{{ .Role }}<|end_header_id|>\n\n{{ .Content }}<|eot_id|>{{ end }}<|start_header_id|>assistant<|end_header_id|>\n\n")
			if err == nil {
				model.Template = tmpl
			}
		}
